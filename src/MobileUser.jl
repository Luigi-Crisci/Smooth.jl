...
function move(m::MobileUser,iterations::Int64)
	...
	for iteration in 1:iterations
		...
		println("Select cluster $cluster_index with $(length(cluster.points))")
		#Select y% subset of cluster waypoints
		num_waypoints = ceil(Int64,length(cluster.points) * m.y)
		println("Num waypoints selected $num_waypoints")
		#Extract subpoint list from the cluster
		vertex_list = random_elements_from_array(collect(1:length(cluster.points)),num_waypoints)
